...
function createBaseServerConfig(): ServerConfig {
  return {
    ...
    availableEditors: [],
    observability: {
      logsDirectoryPath: "/repo/project/.marcode/logs",
...
function createMockEnvironmentApi(input: {
  ...
  const notImplemented = (method: string) => () => {
    throw new Error(`${method} is not implemented in browser test.`);
  };
  return {
...
describe("ChatView timeline estimator parity (full app)", () => {
    ...
    try {
      await vi.waitFor(
        () => {
          ...
          expect(openRequest?.env?.MARCODE_WORKTREE_PATH).toBeUndefined();
        },
        { timeout: 8_000, interval: 16 },
      );
    } finally {
  ...
  it("falls back to the first installed editor when the stored favorite is unavailable", async () => {
    localStorage.setItem("marcode:last-editor", JSON.stringify("vscodium"));
    setDraftThreadWithoutWorktree();
          ...
          expect(dispatchRequest).toMatchObject({
            ...
            bootstrap: {
              ...
              prepareWorktree: {
                ...
                baseBranch: "main",
                branch: expect.stringMatching(/^marcode\/[0-9a-f]{8}$/),
              },
              runSetupScript: true,
            },
          });
  ...
  it("shows the confirm archive action after clicking the archive button", async () => {
    ...
    try {
      ...
      localStorage.removeItem("marcode:client-settings:v1");
      await mounted.cleanup();
    }
  });
  ...
  it("keeps the wide desktop follow-up layout expanded when the footer still fits", async () => {
    const mounted = await mountChatView({
      ...
      snapshot: createSnapshotWithPlanFollowUpPrompt({
        ...
        planMarkdown:
          "# Imaginary Long-Range Plan: MarCode Adaptive Orchestration and Safe-Delay Execution Initiative",
      }),
    });
