...
function buildBlockingCopy(
  ...
  if (uiState === "offline") {
    return {
      description:
        "Your browser is offline, so the web client cannot reach the MarCode server. Reconnect to the network and the app will retry automatically.",
      eyebrow: "Offline",
  ...
  if (status.lastError?.trim()) {
    return {
      description: `${status.lastError} Verify that the MarCode server is running and reachable, then reload the app if needed.`,
      eyebrow: "Connection Error",
      title: "Cannot reach the MarCode server",
    };
  }

  return {
    description:
      "The web client could not complete its initial WebSocket connection to the MarCode server. It will keep retrying in the background.",
    eyebrow: "Connection Error",
    title: "Cannot reach the MarCode server",
  };
}
