...
export default function DiffPanel({ mode = "inline" }: DiffPanelProps) {
  ...
  const selectedPatch = isGitMode
    ? gitWorkingTreeDiffQuery.data?.diff
    : selectedTurn
      ? selectedTurnCheckpointDiff
      : conversationCheckpointDiff;
  const isLoadingDiff = isGitMode ? gitWorkingTreeDiffQuery.isLoading : isLoadingCheckpointDiff;
  const diffError = isGitMode
    ? gitWorkingTreeDiffQuery.error instanceof Error
      ...
      : gitWorkingTreeDiffQuery.error
        ...
        : null
    : checkpointDiffError;
  const isGitDiffTruncated = isGitMode && (gitWorkingTreeDiffQuery.data?.truncated ?? false);
  const hasResolvedPatch = typeof selectedPatch === "string";
  const hasNoNetChanges = hasResolvedPatch && selectedPatch.trim().length === 0;
  const renderablePatch = useMemo(
    () => getRenderablePatch(selectedPatch, `diff-panel:${resolvedTheme}:${diffScope}`),
    [diffScope, resolvedTheme, selectedPatch],
  );
  ...
  useEffect(() => {
    if (isGitMode && activeCwd) {
      void queryClient.invalidateQueries({
        queryKey: gitQueryKeys.workingTreeDiff(activeCwd),
      });
    }
  }, [isGitMode, activeCwd, gitStatusQuery.data, queryClient]);
    ...
    <>
      ...
      <div className="flex shrink-0 items-center gap-1 [-webkit-app-region:no-drag]">
        ...
        >
          ...
          <Toggle
            aria-label="Git working tree diff"
  ...
  return (
    <DiffPanelShell mode={mode} header={headerRow}>
      {!activeThread ? (
      ...
      ) : !isGitMode && orderedTurnDiffSummaries.length === 0 ? (
        <div className="flex flex-1 items-center justify-center px-5 text-center text-xs text-muted-foreground/70">
        ...
        <>
          ...
          >
            ...
            {isGitDiffTruncated && renderablePatch && (
              <div className="px-3 pt-1">
                <p className="text-[11px] text-amber-500/80">
                  Diff output was truncated because it exceeded the size limit.
                </p>
              </div>
            )}
            {!renderablePatch ? (
              isLoadingDiff ? (
                <DiffPanelLoadingState
                  label={isGitMode ? "Loading working tree diff..." : "Loading checkpoint diff..."}
                />
              ) : (
