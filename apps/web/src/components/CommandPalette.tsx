...
function OpenCommandPaletteDialog() {
  ...
  const [addFolderContext, setAddFolderContext] = useState<{
  ...
  const openAddFolderFlow = useCallback(
    (intent: {
      ...
      setAddFolderContext({ onConfirm: intent.onConfirm });
      pushPaletteView({
        addonIcon: <FolderPlusIcon className={ADDON_ICON_CLASS} />,
        groups: [],
        initialQuery: ensureBrowseDirectoryPath(intent.initialPath),
      });
    },
    [],
  );
  ...
  useEffect(() => {
    ...
    if (openIntent.kind === "add-folder") {
      clearOpenIntent();
      openAddFolderFlow({
        environmentId: openIntent.environmentId,
        initialPath: openIntent.initialPath,
        onConfirm: openIntent.onConfirm,
      });
      return;
    }
  }, [clearOpenIntent, openAddFolderFlow, openAddProjectFlow, openIntent]);
  ...
  const handleAddFolder = useCallback(
    async (rawPath: string) => {
      ...
      if (isExplicitRelativeProjectPath(rawPath.trim()) && !currentProjectCwdForBrowse) {
        toastManager.add({
          type: "error",
          title: "Failed to add folder",
      ...
      const absolutePath = resolveProjectPathForDispatch(rawPath, currentProjectCwdForBrowse);
      if (absolutePath.length === 0) return;

      try {
        await addFolderContext.onConfirm(absolutePath);
        setOpen(false);
      } catch (error) {
        toastManager.add({
          type: "error",
  ...
  const isAddFolderMode = addFolderContext !== null;
  ...
  function handleKeyDown(event: KeyboardEvent<HTMLInputElement>): void {
    ...
    if (shouldSubmitBrowsePath) {
      event.preventDefault();
      if (isAddFolderMode) {
        void handleAddFolder(resolvedAddProjectPath);
      } else {
  ...
  const handleOpenProjectFromFileManager = useCallback(async () => {
    ...
    if (isAddFolderMode) {
      await handleAddFolder(pickedPath);
    } else {
          ...
          <CommandPaletteResults
            ...
            {...(relativePathNeedsActiveProject
              ...
              : willCreateProjectPath
                ...
                : isAddFolderMode && isBrowsing
                  ? { emptyStateMessage: "Press Enter to add this folder to the thread." }
                  : {})}
          />
        </CommandPanel>
