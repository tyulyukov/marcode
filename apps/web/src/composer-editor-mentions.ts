...
function forEachPromptSegmentSlice(
  ...
  for (let index = 0; index < prompt.length; index += 1) {
    ...
    if (char !== INLINE_TERMINAL_CONTEXT_PLACEHOLDER && char !== INLINE_JIRA_CONTEXT_PLACEHOLDER) {
      continue;
    }
    ...
    const sliceType =
      char === INLINE_TERMINAL_CONTEXT_PLACEHOLDER ? "terminal-context" : "jira-context";
...
export function splitPromptIntoComposerSegments(
  ...
  forEachPromptSegmentSlice(prompt, (slice) => {
    ...
    } else if (slice.type === "jira-context") {
      segments.push({
        type: "jira-context",
