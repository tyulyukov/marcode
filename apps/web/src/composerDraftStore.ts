...
export const COMPOSER_DRAFT_STORAGE_KEY = "marcode:composer-drafts:v1";
...
interface ComposerDraftStoreState {
  ...
  addJiraTaskContext: (threadRef: ComposerThreadTarget, task: JiraTaskDraft) => void;
  removeJiraTaskContext: (threadRef: ComposerThreadTarget, taskId: string) => void;
  setJiraTaskContexts: (threadRef: ComposerThreadTarget, tasks: JiraTaskDraft[]) => void;
  clearJiraTaskContexts: (threadRef: ComposerThreadTarget) => void;
  addQuotedContext: (threadRef: ComposerThreadTarget, context: QuotedContext) => void;
  removeQuotedContext: (threadRef: ComposerThreadTarget, contextId: string) => void;
  clearQuotedContexts: (threadRef: ComposerThreadTarget) => void;
...
/**
 * Coerce an arbitrary value into a list of `ProviderOptionSelection` entries.
 *
 * Two shapes are supported:
 *   1. The canonical array shape `[{ id, value }, ...]` emitted by the
 *      upstream option-array refactor.
 *   2. A legacy object shape `{ reasoningEffort: "high", fastMode: true }`
 *      which we migrate by promoting each key/value pair to an entry.
 *
 * Values other than strings/booleans are dropped. Empty results return
 * `undefined` so callers can omit the provider entry entirely. This
 * function does not validate ids against the provider's descriptor set —
 * that's the responsibility of the capability layer, which is the source
 * of truth for which option ids are meaningful for a given provider/model.
 * Anything outside the descriptor list is harmless trailing data and will
 * simply be ignored downstream.
 */
...
function normalizePersistedDraftsByThreadId(
  ...
  for (const [threadKeyOrId, draftValue] of Object.entries(
    ...
    nextDraftsByThreadKey[normalizedThreadKey] = persistedEntry;
  }
  ...
  persist(
    (setBase, get) => {
      ...
      return {
        ...
        addJiraTaskContext: (threadRef, task) => {
          const threadKey = resolveComposerDraftKey(get(), threadRef) ?? "";
          if (threadKey.length === 0) {
            return;
          }
          set((state) => {
            const existing = state.draftsByThreadKey[threadKey] ?? createEmptyThreadDraft();
            const dedupKey = jiraTaskDedupKey(task);
            if (existing.jiraTaskContexts.some((t) => jiraTaskDedupKey(t) === dedupKey)) {
              return state;
            }
            return {
              draftsByThreadKey: {
                ...
                [threadKey]: {
                  ...existing,
                  jiraTaskContexts: [...existing.jiraTaskContexts, task],
                },
              },
            };
          });
        },
        removeJiraTaskContext: (threadRef, taskId) => {
          const threadKey = resolveComposerDraftKey(get(), threadRef) ?? "";
          if (threadKey.length === 0) {
            return;
          }
          set((state) => {
            ...
            const nextDraft: ComposerThreadDraftState = {
              ...current,
              jiraTaskContexts: current.jiraTaskContexts.filter((t) => t.id !== taskId),
            };
            const nextDraftsByThreadKey = { ...state.draftsByThreadKey };
            if (shouldRemoveDraft(nextDraft)) {
              delete nextDraftsByThreadKey[threadKey];
        ...
        setJiraTaskContexts: (threadRef, tasks) => {
          const threadKey = resolveComposerDraftKey(get(), threadRef) ?? "";
          if (threadKey.length === 0) {
            return;
          }
          set((state) => {
            const existing = state.draftsByThreadKey[threadKey] ?? createEmptyThreadDraft();
            const nextDraft: ComposerThreadDraftState = { ...existing, jiraTaskContexts: tasks };
            const nextDraftsByThreadKey = { ...state.draftsByThreadKey };
            if (shouldRemoveDraft(nextDraft)) {
              delete nextDraftsByThreadKey[threadKey];
            } else {
        ...
        clearJiraTaskContexts: (threadRef) => {
          const threadKey = resolveComposerDraftKey(get(), threadRef) ?? "";
          if (threadKey.length === 0) {
            return;
          }
          set((state) => {
            const current = state.draftsByThreadKey[threadKey];
            if (!current || current.jiraTaskContexts.length === 0) {
              return state;
            }
            const nextDraft: ComposerThreadDraftState = {
              ...current,
        ...
        addQuotedContext: (threadRef, context) => {
          const threadKey = resolveComposerDraftKey(get(), threadRef) ?? "";
          if (threadKey.length === 0) {
            return;
          }
        ...
        removeQuotedContext: (threadRef, contextId) => {
          const threadKey = resolveComposerDraftKey(get(), threadRef) ?? "";
          if (threadKey.length === 0) {
            return;
          }
