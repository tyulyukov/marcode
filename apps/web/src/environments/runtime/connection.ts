...
import type { KnownEnvironment } from "@marcode/client-runtime";
...
export function createEnvironmentConnection(
  ...
  const runSnapshotRecovery = async (): Promise<void> => {
    const listing = await input.client.orchestration.getListingSnapshot();
    if (disposed) {
      return;
    }
    input.syncListingSnapshot(listing, environmentId);
  };
  ...
  const unsubShell = input.client.orchestration.subscribeShell(
    (item: Parameters<Parameters<WsRpcClient["orchestration"]["subscribeShell"]>[0]>[0]) => {
      if (item.kind === "snapshot") {
        ...
        void runSnapshotRecovery().catch(() => undefined);
