...
describe("TerminalManager", () => {
  ...
  function makeManager(
    ...
    const logsDir = fs.mkdtempSync(path.join(os.tmpdir(), "marcode-terminal-"));
  ...
  it("injects runtime env overrides into spawned terminals", async () => {
    ...
    expect(spawnInput.env.MARCODE_PROJECT_ROOT).toBe("/repo");
    expect(spawnInput.env.MARCODE_WORKTREE_PATH).toBe("/repo/worktree-a");
