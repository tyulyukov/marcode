      ...
      probe: {
        ...
        message:
          openCodeSettings.serverUrl.trim().length > 0
            ? "OpenCode is disabled in MarCode settings. A server URL is configured."
            : "OpenCode is disabled in MarCode settings.",
      },
    });
  }
    ...
    const checkOpenCodeProviderStatus = Effect.fn("checkOpenCodeProviderStatus")(function* (input: {
      ...
      if (!isExternalServer) {
        ...
        if (!version) {
          return fallback(
            new Error(
              `Unable to determine OpenCode version from \`opencode --version\` output. MarCode requires OpenCode v${MINIMUM_OPENCODE_VERSION} or newer.`,
            ),
            null,
          );
        }
