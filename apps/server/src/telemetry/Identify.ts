...
export type TelemetryIdentifierKind = "codex" | "claude" | "anonymous";
 ...
 * 3. <stateDir>/anonymous-id (persisted UUIDv4 fallback)
 */
...
/**
 * getTelemetryIdentifierWithKind - same chain as getTelemetryIdentifier but also exposes
 * which source produced the identifier ("codex" | "claude" | "anonymous").
 */
export const getTelemetryIdentifierWithKind: Effect.Effect<
  { readonly id: string | null; readonly kind: TelemetryIdentifierKind | null },
  never,
  ServerConfig | FileSystem.FileSystem | Path.Path
> = Effect.gen(function* () {
  const codexAccountId = yield* Effect.result(getCodexAccountId);
  if (codexAccountId._tag === "Success") {
    const id = yield* hash(codexAccountId.success);
    return { id, kind: "codex" as const };
  }

  const claudeUserId = yield* Effect.result(getClaudeUserId);
  if (claudeUserId._tag === "Success") {
    const id = yield* hash(claudeUserId.success);
    return { id, kind: "claude" as const };
  }

  const anonymousId = yield* Effect.result(upsertAnonymousId);
  if (anonymousId._tag === "Success") {
    const id = yield* hash(anonymousId.success);
    return { id, kind: "anonymous" as const };
  }

  return { id: null, kind: null };
}).pipe(
  Effect.tapError((error) => Effect.logWarning("Failed to get identifier", { cause: error })),
  Effect.orElseSucceed(() => ({ id: null, kind: null })),
);
