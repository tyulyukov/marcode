...
describe("OrchestrationEngine", () => {
    ...
    const layer = OrchestrationEngineLive.pipe(
      Layer.provide(
        Layer.succeed(ProjectionSnapshotQuery, {
          ...
          getShellSnapshot: () =>
            Effect.die("OrchestrationEngine test should not request the shell snapshot"),
          getProjectShellById: () =>
            Effect.die("OrchestrationEngine test should not request a project shell"),
          getThreadShellById: () =>
            Effect.die("OrchestrationEngine test should not request a thread shell"),
          getThreadDetailById: () =>
            Effect.die("OrchestrationEngine test should not request a thread detail"),
  ...
  it("records command ack duration using the first committed event type", async () => {
    ...
    expect(
      hasMetricSnapshot(snapshots, "marcode_orchestration_command_ack_duration", {
  ...
  it("records failed command dispatches as metric failures", async () => {
    ...
    const snapshots = await system.run(Metric.snapshot);
    expect(
      hasMetricSnapshot(snapshots, "marcode_orchestration_commands_total", {
        commandType: "thread.create",
        aggregateKind: "thread",
        outcome: "failure",
      }),
  ...
  it("stores completed checkpoint summaries even when no files changed", async () => {
    ...
    await system.run(
      engine.dispatch({
        ...
        checkpointRef: asCheckpointRef("refs/marcode/checkpoints/thread-turn-diff/turn/1"),
        status: "ready",
        files: [],
