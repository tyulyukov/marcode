...
import { makeProviderUpdater } from "./provider/providerUpdater.ts";
...
import { browseDirectories } from "./workspaceEntries.ts";
...
const makeWsRpcLayer = (currentSessionId: AuthSessionId) =>
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      const toJiraRpcError = (cause: unknown) =>
        new JiraRpcError({
          message: cause instanceof Error ? cause.message : "Jira operation failed",
        });
      ...
      return WsRpcGroup.of({
        ...
        [WS_METHODS.projectsBrowseDirectories]: (input) =>
          observeRpcEffect(
            WS_METHODS.projectsBrowseDirectories,
            Effect.tryPromise({
              try: () => browseDirectories(input),
              catch: (cause) =>
        ...
        [WS_METHODS.gitWorkingTreeDiff]: (input) =>
          observeRpcEffect(WS_METHODS.gitWorkingTreeDiff, git.readWorkingTreeDiff(input.cwd), {
            "rpc.aggregate": "git",
          }),
        ...
        [WS_METHODS.jiraGetConnectionStatus]: (_input) =>
          observeRpcEffect(
            WS_METHODS.jiraGetConnectionStatus,
            jiraApiClient.getConnectionStatus.pipe(Effect.mapError(toJiraRpcError)),
            { "rpc.aggregate": "jira" },
          ),
        [WS_METHODS.jiraDisconnect]: (_input) =>
          observeRpcEffect(
            WS_METHODS.jiraDisconnect,
            jiraApiClient.disconnect.pipe(Effect.mapError(toJiraRpcError)),
            { "rpc.aggregate": "jira" },
          ),
        [WS_METHODS.jiraListSites]: (_input) =>
          observeRpcEffect(
            WS_METHODS.jiraListSites,
            jiraApiClient.getAccessibleResources.pipe(
              Effect.map((sites) => ({ sites: Array.from(sites) })),
              Effect.mapError(toJiraRpcError),
            ),
            { "rpc.aggregate": "jira" },
          ),
        [WS_METHODS.jiraListBoards]: (input) =>
          observeRpcEffect(
            WS_METHODS.jiraListBoards,
            jiraApiClient.listBoards(input).pipe(Effect.mapError(toJiraRpcError)),
            { "rpc.aggregate": "jira" },
          ),
        [WS_METHODS.jiraListSprints]: (input) =>
          observeRpcEffect(
            WS_METHODS.jiraListSprints,
            jiraApiClient.listSprints(input).pipe(Effect.mapError(toJiraRpcError)),
            { "rpc.aggregate": "jira" },
          ),
        [WS_METHODS.jiraListIssues]: (input) =>
          observeRpcEffect(
            WS_METHODS.jiraListIssues,
            jiraApiClient.listIssues(input).pipe(Effect.mapError(toJiraRpcError)),
            { "rpc.aggregate": "jira" },
          ),
        [WS_METHODS.jiraGetIssue]: (input) =>
          observeRpcEffect(
            WS_METHODS.jiraGetIssue,
            jiraApiClient.getIssue(input).pipe(Effect.mapError(toJiraRpcError)),
            { "rpc.aggregate": "jira" },
          ),
        [WS_METHODS.jiraGetAttachment]: (input) =>
          observeRpcEffect(
            WS_METHODS.jiraGetAttachment,
            jiraApiClient.getAttachment(input).pipe(Effect.mapError(toJiraRpcError)),
            { "rpc.aggregate": "jira" },
          ),
        [WS_METHODS.subscribeJiraConnectionStatus]: (_input) =>
          observeRpcStreamEffect(
            WS_METHODS.subscribeJiraConnectionStatus,
            Effect.succeed(
              jiraTokenService.streamChanges.pipe(
                Stream.mapEffect(() =>
                  jiraApiClient.getConnectionStatus.pipe(Effect.mapError(toJiraRpcError)),
                ),
              ),
