...
const baseDirFlag = Flag.string("base-dir").pipe(
  Flag.withDescription("Base directory path (equivalent to MARCODE_HOME)."),
  Flag.optional,
);
...
const serveCommand = Command.make("serve", { ...sharedServerCommandFlags }).pipe(
  Command.withDescription(
    "Run the MarCode server without opening a browser and print headless pairing details.",
  ),
  Command.withHandler((flags) =>
...
export const cli = Command.make("marcode", { ...sharedServerCommandFlags }).pipe(
  Command.withDescription("Run the MarCode server."),
  Command.withHandler((flags) => runServerCommand(flags)),
