...
const BootstrapEnvelopeSchema = Schema.Struct({
  ...
  marcodeHome: Schema.optional(Schema.String),
...
interface CliInput {
  ...
  readonly marcodeHome: Option.Option<string>;
...
const CliEnvConfig = Config.all({
  ...
  authToken: Config.string("MARCODE_AUTH_TOKEN").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
      ...
      const baseDir = yield* resolveBaseDir(
        Option.getOrUndefined(
          resolveOptionPrecedence(
            ...
            Option.flatMap(bootstrapEnvelope, (bootstrap) =>
              Option.fromUndefinedOr(bootstrap.marcodeHome),
            ),
          ),
        ),
      );
...
const makeServerRuntimeProgram = (input: CliInput) =>
  Effect.gen(function* () {
    ...
    yield* Effect.logInfo("MarCode running", {
      ...safeConfig,
...
const marcodeHomeFlag = Flag.string("home-dir").pipe(
...
export const marcodeCli = Command.make("marcode", {
  mode: modeFlag,
  port: portFlag,
  host: hostFlag,
  marcodeHome: marcodeHomeFlag,
  devUrl: devUrlFlag,
  ...
  Command.withDescription("Run the MarCode server."),
