...
export const DEFAULT_MARCODE_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(homedir(), ".marcode"),
);
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("MARCODE_PORT_OFFSET"),
  devInstance: optionalStringConfig("MARCODE_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid MARCODE_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `MARCODE_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric MARCODE_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed MARCODE_DEV_INSTANCE=${seed}` };
}
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_MARCODE_HOME;
  });
}
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const resolvedBaseDir = yield* resolveBaseDir(marcodeHome);
    ...
    const output: NodeJS.ProcessEnv = {
      ...
      MARCODE_HOME: resolvedBaseDir,
    };

    if (!isDesktopMode) {
      output.MARCODE_PORT = String(serverPort);
      output.VITE_WS_URL = `ws://localhost:${serverPort}`;
    } else {
      delete output.MARCODE_PORT;
      delete output.VITE_WS_URL;
      delete output.MARCODE_AUTH_TOKEN;
      delete output.MARCODE_MODE;
      delete output.MARCODE_NO_BROWSER;
      delete output.MARCODE_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.MARCODE_HOST = host;
    }

    if (!isDesktopMode && authToken !== undefined) {
      output.MARCODE_AUTH_TOKEN = authToken;
    } else if (!isDesktopMode) {
      delete output.MARCODE_AUTH_TOKEN;
    }

    if (!isDesktopMode && noBrowser !== undefined) {
      output.MARCODE_NO_BROWSER = noBrowser ? "1" : "0";
    } else if (!isDesktopMode) {
      delete output.MARCODE_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.MARCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.MARCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.MARCODE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.MARCODE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.MARCODE_MODE = "web";
      delete output.MARCODE_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read MARCODE_PORT_OFFSET/MARCODE_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    const envOverrides = {
      noBrowser: readOptionalBooleanEnv("MARCODE_NO_BROWSER"),
      autoBootstrapProjectFromCwd: readOptionalBooleanEnv(
        "MARCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD",
      ),
      logWebSocketEvents: readOptionalBooleanEnv("MARCODE_LOG_WS_EVENTS"),
    };
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.MARCODE_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.MARCODE_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  marcodeHome: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all MarCode data (equivalent to MARCODE_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("MARCODE_HOME")),
  ),
  authToken: Flag.string("auth-token").pipe(
    Flag.withDescription("Auth token (forwards to MARCODE_AUTH_TOKEN)."),
    Flag.withAlias("token"),
    Flag.withFallbackConfig(optionalStringConfig("MARCODE_AUTH_TOKEN")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to MARCODE_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("MARCODE_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to MARCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("MARCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to MARCODE_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("MARCODE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to MARCODE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("MARCODE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to MARCODE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("MARCODE_PORT")),
  ),
