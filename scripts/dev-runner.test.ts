...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  describe("resolveOffset", () => {
    it.effect("uses explicit MARCODE_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "MARCODE_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid MARCODE_PORT_OFFSET"));
      }),
    );
  });
  ...
  describe("createDevRunnerEnv", () => {
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          ...
          serverOffset: 0,
          webOffset: 0,
          marcodeHome: "/tmp/custom-marcode",
          authToken: "secret",
          noBrowser: true,
        ...
        assert.equal(env.MARCODE_NO_BROWSER, "1");
        assert.equal(env.MARCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.MARCODE_LOG_WS_EVENTS, "1");
        assert.equal(env.MARCODE_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            MARCODE_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          marcodeHome: undefined,
        ...
        assert.equal(env.MARCODE_MODE, "web");
        assert.equal(env.MARCODE_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.MARCODE_LOG_WS_EVENTS, "0");
      }),
    );
    ...
    it.effect("does not export backend bootstrap env for dev:desktop", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          ...
          baseEnv: {
            MARCODE_PORT: "3773",
            MARCODE_AUTH_TOKEN: "stale-token",
            MARCODE_MODE: "web",
            MARCODE_NO_BROWSER: "0",
            MARCODE_HOST: "0.0.0.0",
        ...
        assert.equal(env.MARCODE_PORT, undefined);
        assert.equal(env.MARCODE_AUTH_TOKEN, undefined);
        assert.equal(env.MARCODE_MODE, undefined);
        assert.equal(env.MARCODE_NO_BROWSER, undefined);
        assert.equal(env.MARCODE_HOST, undefined);
