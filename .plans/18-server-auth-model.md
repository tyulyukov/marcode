...
Define the long-term server auth architecture for MarCode before first-class remote environments ship.
...
Every privileged surface of the MarCode server must go through the same auth policy engine:
...
Participants:
  ...
  MarCodeServer      = local backend child process
...
DesktopMain -> MarCodeServer : spawn server (--bootstrap-fd ...)
DesktopMain -> MarCodeServer : send desktop bootstrap envelope
note over MarCodeServer : policy = DesktopManagedLocalPolicy
note over MarCodeServer : allowed pairing = desktop-bootstrap only

Frontend -> DesktopMain : request local bootstrap grant
DesktopMain --> Frontend : short-lived desktop bootstrap grant

Frontend -> MarCodeServer : POST /api/auth/bootstrap
MarCodeServer -> MarCodeServer : validate desktop bootstrap grant
MarCodeServer -> MarCodeServer : create browser session
MarCodeServer --> Frontend : Set-Cookie: session=...

Frontend -> MarCodeServer : GET /ws + authenticated cookie
MarCodeServer -> MarCodeServer : validate cookie session
MarCodeServer --> Frontend : websocket accepted
```
...
```text
Participants:
  UserShell     = npx marcode launcher
  MarCodeServer      = standalone local server
  Browser       = browser tab

UserShell -> MarCodeServer : start server
MarCodeServer -> MarCodeServer : getOrCreate("server-signing-key")
note over MarCodeServer : policy = LoopbackBrowserPolicy

UserShell -> MarCodeServer : issue one-time pairing token
MarCodeServer --> UserShell : pairing URL or pairing token

UserShell --> Browser : open /pair?token=...

Browser -> MarCodeServer : GET /pair?token=...
MarCodeServer -> MarCodeServer : validate one-time token
MarCodeServer -> MarCodeServer : create browser session
MarCodeServer --> Browser : Set-Cookie: session=...
MarCodeServer --> Browser : redirect to app

Browser -> MarCodeServer : GET /ws + authenticated cookie
MarCodeServer --> Browser : websocket accepted
```
...
Participants:
  ...
  MarCodeServer      = MarCode server
  PhoneBrowser  = mobile browser

DesktopUser -> DesktopMain : enable remote access via tunnel
DesktopMain -> MarCodeServer : switch policy to RemoteReachablePolicy
DesktopMain -> Tunnel : publish local MarCode endpoint
Tunnel --> DesktopMain : public https/wss URL

DesktopMain -> MarCodeServer : issue one-time pairing token
MarCodeServer --> DesktopMain : pairing token
...
Tunnel -> MarCodeServer : forward request
MarCodeServer -> MarCodeServer : validate one-time token
MarCodeServer -> MarCodeServer : create mobile browser session
MarCodeServer --> PhoneBrowser : Set-Cookie: session=...
MarCodeServer --> PhoneBrowser : redirect to app

PhoneBrowser -> Tunnel : GET /ws + authenticated cookie
Tunnel -> MarCodeServer : forward websocket upgrade
MarCodeServer --> PhoneBrowser : websocket accepted
```
...
DesktopUser -> MarCodeServer : enable private-network access
MarCodeServer -> MarCodeServer : switch policy to RemoteReachablePolicy
DesktopUser -> MarCodeServer : issue one-time pairing token
MarCodeServer --> DesktopUser : pairing URL / QR

DesktopUser -> PhoneBrowser : open private-network URL
PhoneBrowser -> PrivateNet : GET http(s)://private-host/pair?token=...
PrivateNet -> MarCodeServer : route request
MarCodeServer -> MarCodeServer : validate one-time token
MarCodeServer -> MarCodeServer : create mobile browser session
MarCodeServer --> PhoneBrowser : Set-Cookie: session=...
MarCodeServer --> PhoneBrowser : redirect to app
...
PrivateNet -> MarCodeServer : websocket upgrade
MarCodeServer --> PhoneBrowser : websocket accepted
```
...
Participants:
  ...
  RemoteMarCode      = remote MarCode server
  Frontend      = desktop renderer

DesktopUser -> DesktopMain : add SSH host
DesktopMain -> SSH : connect to remote host
SSH -> RemoteHost : probe environment / verify marcode availability
DesktopMain -> SSH : run remote launch command
SSH -> RemoteHost : marcode remote launch --json
RemoteHost -> RemoteMarCode : start or reuse server
RemoteMarCode --> RemoteHost : port + environment metadata
...
note over RemoteMarCode : policy = RemoteReachablePolicy
note over DesktopMain,RemoteMarCode : desktop may use a trusted bootstrap flow here

Frontend -> DesktopMain : request bootstrap for selected environment
DesktopMain --> Frontend : short-lived bootstrap grant

Frontend -> RemoteMarCode : POST /api/auth/bootstrap via forwarded port
RemoteMarCode -> RemoteMarCode : validate bootstrap grant
RemoteMarCode -> RemoteMarCode : create browser session
RemoteMarCode --> Frontend : Set-Cookie: session=...

Frontend -> RemoteMarCode : GET /ws + authenticated cookie
RemoteMarCode --> Frontend : websocket accepted
```
