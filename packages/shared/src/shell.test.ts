...
describe("extractPathFromShellOutput", () => {
  it("extracts the path between capture markers", () => {
    expect(
      extractPathFromShellOutput(
        "__MARCODE_PATH_START__\n/opt/homebrew/bin:/usr/bin\n__MARCODE_PATH_END__\n",
      ),
...
describe("readPathFromLoginShell", () => {
  it("uses a shell-agnostic printenv PATH probe", () => {
    ...
    expect(args?.[1]).toContain("__MARCODE_ENV_PATH_START__");
    expect(args?.[1]).toContain("__MARCODE_ENV_PATH_END__");
...
describe("readEnvironmentFromLoginShell", () => {
  it("extracts multiple environment variables from a login shell command", () => {
    ...
    >(() =>
      [
        ...
        "/a:/b",
        "__MARCODE_ENV_PATH_END__",
        "__MARCODE_ENV_SSH_AUTH_SOCK_START__",
        "/tmp/secretive.sock",
        "__MARCODE_ENV_SSH_AUTH_SOCK_END__",
      ].join("\n"),
    );
  ...
  it("preserves surrounding whitespace in captured values", () => {
    ...
    >(() =>
      [
        "__MARCODE_ENV_CUSTOM_VAR_START__",
        "  padded value  ",
        "__MARCODE_ENV_CUSTOM_VAR_END__",
      ].join("\n"),
    );
