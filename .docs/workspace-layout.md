...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `marcode` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@marcode/shared/git`, `@marcode/shared/DrainableWorker`) — no barrel index.
