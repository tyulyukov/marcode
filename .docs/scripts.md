...
- Dev commands default `MARCODE_STATE_DIR` to `~/.marcode/dev` to keep dev state isolated from desktop/prod state.
- Override server CLI-equivalent flags from root dev commands with `--`, for example:
  `bun run dev -- --base-dir ~/.marcode-2`
...
- Desktop packaging includes `apps/server/dist` (the `marcode` backend) and starts it on loopback with an auth token for WebSocket/API traffic.
...
- Example: `MARCODE_DEV_INSTANCE=branch-a bun run dev:desktop`
